...
ticker_input = st.text_input("Enter Tickers (e.g., TSLA, BY6.GETTEX, VUAG.LSE, ASML.AMS)", "TSLA, BY6.GETTEX, VUAG.LSE, ASML.AMS")
...
if st.button("Run Analysis"):
    ...
    t212_map = {".LSE": ".L", ".GETTEX": ".MU", ".XETR": ".DE", ".AMS": ".AS", ".PAR": ".PA", ".MIL": ".MI"}
    ...
    for original in raw_tickers:
        ...
        try:
            with st.spinner(f"Analyzing {original}..."):
                ...
                info, bs, is_stmt = stock.info, stock.balance_sheet, stock.financials
                
                # Fetch Financial Data
                assets = bs.loc['Total Assets'].iloc[0] if 'Total Assets' in bs.index else 1
                rev = is_stmt.loc['Total Revenue'].iloc[0] if 'Total Revenue' in is_stmt.index else 1
                debt = bs.loc['Total Debt'].iloc[0] if 'Total Debt' in bs.index else 0
                spot_mcap = info.get('marketCap', 0)
                avg_mc = get_stable_mcap(stock)
                
                # Interest income
                int_inc_raw = is_stmt.loc['Interest Income'].iloc[0] if 'Interest Income' in is_stmt.index else np.nan
                has_int_data = pd.notnull(int_inc_raw)
                impure_rev = ((int_inc_raw if has_int_data else 0) / rev) * 100
                ...
                # Currency Normalizer
                if debt_avg_mc > 500: debt_avg_mc = debt_assets * 1.1 
                if debt_spot_mc > 500: debt_spot_mc = debt_assets * 1.1
